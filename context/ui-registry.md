...
Empty state (**Feature 29**, upgraded from a bare dashed-border text box): the locked Empty State pattern, `BedDouble` icon (matching `RoomSelectionSection`'s `BedDoubleIcon` on the frontend for the analogous "No rooms available" case), heading "No room types yet", inline hand-rolled (no shared `EmptyState` component exists in `frontend-admin` — same approach `HotelsListPage`/`BookingsListPage` already use), wrapped in a `rounded-2xl border border-border-default bg-surface` card shell matching this same file's "add new" form card above it. `RateOverrideManager`'s own "No seasonal pricing..." line was deliberately left as plain muted text during the same audit — nested two levels deep inside an already-labeled accordion sub-section, same reasoning as the Admin Dashboard's `EmptyRow` precedent below.
...
`frontend` has a shared `components/common/EmptyState.tsx` component (icon/heading/body/action props) — every `frontend` usage imports it: search ("No hotels match these filters"), favorites, compare, bookings list, hotel-details room selection + not-found, checkout not-found, booking detail not-found, review page not-found/not-eligible, reviews section. `frontend-admin` has **no shared component** — every usage hand-rolls the identical classes inline instead (`size-10 text-text-faint` icon / `text-base font-medium text-text-muted` heading / `max-w-xs text-center text-sm text-text-faint` body): `HotelsListPage` (`Building2` icon), `BookingsListPage` (`CalendarRange` icon), `RoomTypesSection` (**Feature 29**, `BedDouble` icon — deliberately matching `frontend`'s `BedDoubleIcon` choice for the same "no room types/rooms" concept, first explicit cross-app icon-consistency call). If a shared `frontend-admin` primitive is ever introduced, these three are the call sites to consolidate.
