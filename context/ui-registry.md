...
### Checkout (CheckoutPageContent / BookingSummaryCard)

File: `frontend/features/booking/components/{CheckoutPageContent,BookingSummaryCard,CheckoutSkeleton}.tsx`
App: frontend
Last updated: 2026-07-12 (Feature 19)
...
Payment panel (this feature only — real Stripe Elements is Feature 21's job): centered `CreditCardIcon` + "Your room is reserved and awaiting payment. Stripe checkout is coming soon." + a disabled Primary Button "Pay Now" — same not-yet-wired placeholder pattern `RoomTypeCard`'s Reserve button used pre-Feature-19. Feature 21 replaces only this panel's body, the grid/rail structure stays.
