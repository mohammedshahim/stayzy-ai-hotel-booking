...
Segment box (all three — Destination/Date/Guests alike): `m-2 rounded-xl border border-border-default bg-subtle px-4 py-2.5 transition-colors`, active-state border+ring in `accent-border`. **2026-07-19 fix — never add `w-full` to a segment box.** `DateRangePicker` and `GuestsRoomsPicker` (both `PopoverTrigger`s, hence explicit sizing where the `DestinationInput` `div` gets stretch for free) each carried `w-full` alongside `m-2`: `w-full` resolves to 100% of the parent *before* margins apply, so both boxes overhung their margin box by 16px and rendered wider than Destination in any stacked layout (measured 285 vs 301 at 375px). As a stretched flex/grid child each box already fills its track, so `w-full` is never needed here. Affected this widget, `RoomSelectionSection`, and `SearchBar` simultaneously; fixed once in the two shared components. — `focus-within:` for the Destination `div` (a real `<input>` inside), `focus-visible:` + `aria-expanded:` for the Date/Guests `PopoverTrigger`s (base-ui sets `aria-expanded="true"` on a trigger while its popover is open, so the box stays highlighted for the whole time it's open, not just the instant it's focused).
...
Date popover content: `w-auto border border-border-default bg-elevated p-4 shadow-elevated`, wrapping `components/ui/calendar.tsx` in `mode="range"` `numberOfMonths={2}` — no new custom calendar logic, dates before today disabled via `disabled={{ before: new Date() }}`. Since a post-Feature-19 fix, `dateRange` initializes to `defaultDateRange()` (`frontend/lib/date.ts`, a lazy `useState` initializer) rather than `undefined` — the segment shows a real "[Today] – [Tomorrow]" label at rest instead of the "Add dates" placeholder, and `handleSearch` always carries `checkIn`/`checkOut` into the `/search` URL as a result (previously omitted entirely when the user never touched the picker). `frontend/lib/date.ts` mirrors `backend/src/utils/date.ts`'s `todayIso()`/`tomorrowIso()` exactly — any new date-defaulting need on the frontend should import from there rather than re-deriving today/tomorrow locally. **2026-07-19**: `handleSearch` no longer hand-rolls its `URLSearchParams` — it builds `{ ...DEFAULT_STATE, ...fields }` and runs it through `serializeSearchState`, the same helper `/search` parses back, so the two formats can't drift. Date ↔ ISO-string conversion goes through `lib/date.ts`'s `toDateRange()` / `toIsoDates()` (both use an explicit `T00:00:00` to stay local-midnight; bare `new Date("2026-03-04")` parses as UTC and shifts a day in negative offsets).
...
Grid variant: image box `aspect-[4/3] w-full`, body below
List variant (`variant="list"`): image box is `aspect-[4/3] w-full sm:aspect-auto sm:w-56 md:w-64 sm:shrink-0`, body fills the remaining width. **Feature 30 fix**: below `sm:` the list variant now stacks (`flex-col`, full-width image on top) instead of always being `flex-row` with a hardcoded `w-56` image — at 375px the old fixed-row layout squished the text column to the point of clipping hotel names/tags/price (this variant is used both by `/search`'s List view toggle and by `MapView`'s side list, so the bug hit both). Image no longer carries its own `rounded-t-2xl`/`rounded-l-2xl` — the outer wrapper's `overflow-hidden rounded-2xl` already clips it regardless of stack direction, same simpler precedent `RoomTypeCard` uses (removes the need for breakpoint-conditional rounding entirely).
...
**Fixed 2026-07-28 — the image box now follows the Photo Box pattern (see Approved Patterns below), which this bug is what locked it in.** The box carried `aspect-[4/3]` but the `<img>` inside it was in flow with `h-full w-full`, so every portrait photo overrode the ratio and grew its own card: measured at 1440px, a 512×768 seed photo rendered 291×437 in a grid whose landscape cards were 291×218, leaving the results grid visibly ragged. The image is now `absolute inset-0`, the box gained `overflow-hidden bg-subtle`, and `src={hotel.image ?? ""}` became a conditional render (an empty `src` re-requests the page URL and paints a broken-image icon). Every card now measures identically per breakpoint regardless of the photo's intrinsic size — verified in the running app at 1440/820/390px in both Grid and List view. `HotelCardSkeleton`'s list variant was corrected in the same pass from `flex-row`/`w-56 sm:w-64` to the card's real `flex-col sm:flex-row`/`sm:w-56 md:w-64`, so the placeholder occupies the space the loaded card will and the results don't jump on load. The same construction was carried by seven other components and **all were converted to the Photo Box pattern in the same pass** — see that pattern's adoption note below for the list and what was measured.

**Feature 28** added `HotelCardSkeleton.tsx` (grid/list-aware, coarse two-bar fidelity — image block + title bar + price bar, not every rating/amenity sub-detail) and `SearchResultsSkeleton.tsx` (renders `RESULTS_PER_PAGE`=9 of them in the same grid/list wrapper the real results use, plus a single `h-[32rem]` pulsing rectangle for Map view's first load, since a map has no internal structure worth mirroring). Wired into `SearchPageContent`'s `isLoading && isEmpty` branch only — the existing `isLoading && !isEmpty` dim-to-`opacity-60` behavior for filter refetches is untouched.
...
**Feature 28** added `FavoritesSkeleton.tsx` — 6 placeholder cards (fixed count, since there's no page size to match) in the same `grid gap-5 sm:grid-cols-2 xl:grid-cols-3` wrapper, each card a coarse image-block + 2 text-bar shape matching `HotelCardSkeleton`'s fidelity level. Replaces the old "Loading favorites…" text in `FavoritesPageContent`'s `isLoading` branch.
...
CompareTray: matches the locked Floating Compare Tray pattern exactly (`fixed bottom-4 inset-x-0 mx-auto max-w-3xl`, `bg-surface rounded-2xl border border-border-default shadow-elevated px-5 py-4`). Content: `-space-x-2` overlapping `h-9 w-9 rounded-full border-2 border-surface` thumbnails, `"{n} hotel(s) selected"` text, a Primary Button `render={<Link href="/compare" />}`, then a dismiss `XIcon` button (`clear()`s the whole selection — see `architecture.md`'s Feature 18 decision on why dismiss isn't a purely-visual hide). Mounted once in `app/layout.tsx`, renders `null` when the selection is empty. **Feature 30 fix**: inner row is now `flex-col gap-3 sm:flex-row sm:items-center` (avatars+label grouped in their own row, Compare+close grouped in a second row that's full-width below `sm:`) instead of a single always-in-a-row flex — at mobile widths the old single row crowded avatars/label/button/close with no wrap fallback. Reuses the same stack-then-row convention as `RoomTypeCard`/`BookingListCard`, not a bespoke shrink-to-fit. **2026-07-19 fixes** (two, both about horizontal balance above `sm:`): (1) the avatars+label group gained `flex-1` — without it neither group grew, so both clustered against the left edge with dead space trailing to the right, and the `flex-1` already on the label `<span>` was inert because its parent never grew. (2) the outer wrapper is now `w-full max-w-3xl sm:w-fit` rather than always `max-w-3xl` — the pill hugs its content and grows as hotels are added (capped at `max-w-3xl`), instead of a single selected hotel leaving a wide gap between the label and the Compare button. Stays full-width below `sm:`, where the content stacks.
...
CompareTable (`/compare` page): real `<table>`, matching `ui-rules.md`'s Compare Table spec — wrapper `overflow-x-auto rounded-2xl border border-border-default bg-surface`, every hotel `<th>`/`<td>` `min-w-[16rem]`, row-label column `sticky left-0 bg-surface`. **2026-07-19 fix — both cell classes now carry an explicit `text-left`.** `<th>` defaults to `text-align: center`, so the header cells inherited centering while every `<td>` below them was left-aligned: the city/country line rendered centered under a `text-left` hotel name, and the "Hotel" label was centered against left-aligned "Price"/"Rating"/"Amenities"/"Cancellation". Any new row added to this table must keep `text-left` on its cells. Note the column widths were **not** the problem and were not changed — measured equal at every hotel count (493/493 at two, 329×3, 256×4), since `w-full` + `min-w-[16rem]` already satisfies the spec. Header row: hotel photo (`aspect-[4/3]`, an `XIcon` remove button top-right) + name (links to `/hotels/[id]`) + city/country. Body rows: Price (`"from $X/night"`), Rating (`StarRating` + `GuestRatingBadge` + review count), Amenities (Amenity/Skill Tag chips, same as `HotelCard`), Cancellation (same label/body pairing as `PoliciesSection`'s cancellation row: `"Free cancellation"`/`"Cancellation policy"` + the policy text). Reserved AI summary slot below the table: `hidden rounded-2xl border border-border-default bg-elevated p-5` per `ui-rules.md` — static markup only, no logic, until the AI phase.
...
**Feature 28** added `CompareSkeleton.tsx` — same `<table>` structure as `CompareTable` (sticky row-label column, one `<th>`/`<td>` column per hotel), but the column count is driven by the real `ids.length` from `useCompareSelection()` rather than a guessed fixed number, since that count is known synchronously before the hotel data resolves. Wired into `ComparePageContent`'s `isLoading` branch, replacing the old "Loading comparison…" text.
...
AppShell: plain flex wrapper — `Sidebar` + a `min-w-0 flex-1` column (`Topbar` then `mx-auto max-w-7xl px-4 py-8 lg:px-6 lg:py-10` content, matching the `frontend-admin` Content Width spec). **Feature 30 fix**: `min-w-0` added to the `flex-1` column — without it, a flex item won't shrink below its content's intrinsic width, so at 768px the whole page (sidebar included) was growing to whatever width the widest table needed (1041px on Hotels, 1263px on Bookings) instead of containing the overflow to the table's own `overflow-x-auto` scroll region. One-line fix, resolves it for every current and future admin page rendered inside this shell, not just the two tables that surfaced it. `frontend-admin` is scoped to tablet+desktop only (no phone-width requirement, no hamburger/drawer for `Sidebar`) — an explicit developer decision during `/architect`, not an oversight.
...
**Feature 28** added `HotelsTableSkeleton.tsx` — 6 fixed skeleton `TableRow`s (not `PAGE_SIZE`=20, since the real row count is unknown until data loads and a full 20-row skeleton reads as excessive), each cell a pulsing bar sized to that column's typical content width. Replaces the old "Loading hotels..." text row.
...
Empty state (**Feature 29**, upgraded from a bare dashed-border text box; now uses `frontend-admin`'s shared `EmptyState` component — see below): `BedDouble` icon (matching `RoomSelectionSection`'s `BedDoubleIcon` on the frontend for the analogous "No rooms available" case), heading "No room types yet", wrapped in a `rounded-2xl border border-border-default bg-surface` card shell matching this same file's "add new" form card above it. `RateOverrideManager`'s own "No seasonal pricing..." line was deliberately left as plain muted text during the same audit — nested two levels deep inside an already-labeled accordion sub-section, same reasoning as the Admin Dashboard's `EmptyRow` precedent below.
...
BookingListCard: same list-row anatomy as `RoomTypeCard` (`flex flex-col overflow-hidden rounded-2xl border border-border-default bg-elevated shadow-card sm:flex-row`, image `aspect-[4/3] sm:aspect-auto` per the Photo Box pattern, `ImageOffIcon` fallback `absolute inset-0` over the box's own `bg-subtle`), sized `sm:w-40` (narrower than `RoomTypeCard`'s `sm:w-48` — this row carries less content). Unlike `RoomTypeCard`, the whole card is one `Link` to `/bookings/[id]` with a `hover:border-border-subtle` transition — same whole-card-clickable precedent as `HotelCard`/`SimilarHotelCard`. Header inside the card is `flex items-start justify-between gap-3`: hotel name/city,country left, `BookingStatusBadge` right — same left-content/right-badge pairing `HotelsListPage`'s table rows use for Hotel Status Badge.
...
**Feature 28** added `BookingsListSkeleton.tsx` — 6 placeholder rows (fixed count) in the same `flex flex-col gap-4` stack, each row's shape mirroring `BookingListCard`'s image+text anatomy. Replaces the old "Loading bookings…" text in `BookingsPageContent`'s `isLoading` branch.
...
**Feature 28** added `BookingsTableSkeleton.tsx` — same 6-fixed-row approach as `HotelsTableSkeleton`, shaped to this table's 6-column layout (guest two-line block, hotel thumbnail+text, room type, dates, total, status pill). Replaces the old "Loading bookings..." text row.
...
**Feature 28** added `DashboardSkeleton.tsx` — mirrors this page's full layout: 4 `StatCardSkeleton`s in the same grid, then the two `lg:grid-cols-2` section-card rows, each `SectionCardSkeleton` keeping its real `<h2>` title (not a skeleton bar) with 4 skeleton feed rows below it. Replaces the old "Loading dashboard..." text; closes the gap flagged in this entry's own Feature 27 notes.

### SearchBar (`/search`)

File: `frontend/features/search/components/SearchBar.tsx`
App: frontend
Last updated: 2026-07-19

Wrapper: `rounded-2xl border border-border-default bg-surface p-3 shadow-elevated` — same surface recipe as `HeroSearchWidget` (`p-3` rather than `p-5`, since this sits above results rather than in a hero).
Segment row: `flex-col md:grid md:grid-cols-[1fr_1fr_1fr_auto] md:items-stretch lg:grid-cols-[2fr_1fr_1fr_auto]`. Grid, not `HeroSearchWidget`'s flex — destination gets `2fr` at `lg:` because a city name needs more room than "Aug 10 – Aug 14" (measured: 340px → 519px at 1280). Tablet deliberately stays `1fr_1fr_1fr_auto`: `2fr` at 768px squeezes dates/guests to ~123px and wraps the bar to double height, so the weighting only pays off once there's room for it. `md:` (not `HeroSearchWidget`'s `lg:`) for the row breakpoint — a stacked bar on the results page pushes hotels below the fold in a way it doesn't on a hero.
Segments: reuses `DestinationInput` / `DateRangePicker` / `GuestsRoomsPicker` unchanged and with no `className` — the grid owns the column widths now, so the old `lg:flex-1` per-segment override is gone.
Search button: Primary Button pattern, `h-11 w-full md:h-auto md:w-auto` inside an `m-2 flex` cell. `md:h-auto` + the row's `md:items-stretch` is what makes the button match the segments' height — with a fixed `h-11` against 62px-tall segments it bottom-aligned and sat 18px low.
Collapsed pill (below `sm:`): `flex w-full items-center gap-2 rounded-xl px-2 py-1.5 text-left text-sm text-text-secondary hover:bg-subtle sm:hidden`, a `SearchIcon` + `truncate`d "Paris · Aug 10 – Aug 14 · 5 guests" summary that toggles `isExpanded`. Deliberately an inline expanding panel, not a sheet/drawer — no Sheet primitive exists in `components/ui/`, and building one was out of scope. Above `sm:` the fields are always visible and the pill is hidden.
Incomplete-range hint: `px-2 pb-1 text-xs text-text-muted sm:px-4`, rendered only when the draft range is unsearchable; the Search button is `disabled` at the same time.
Notes: hydrates from `useSearchState().state` and holds edits as **draft** state — nothing refetches until Search is pressed (matches `HeroSearchWidget`'s explicit-submit UX; contrast `FilterSidebar`, which applies on every change). Because the draft would otherwise go stale when the URL moves underneath it (back/forward, a `TrendingDestinations` link), it reseeds via React's "adjust state during render" pattern keyed on a signature of the six core params — not a `useEffect`, which would flash the old values for a frame. Submits with `{ history: "push" }` so a search is a real back-stack entry; filters/sort/view keep the default `replace`. **Do not add a `key` on this component to force resync** — that would remount the pickers and close any open popover.

### Profile Page (ProfilePageContent / AvatarNamePanel / EmailPanel / PasswordPanel / ChangePasswordForm / SetPasswordForm)

File: `frontend/features/profile/components/{ProfilePageContent,AvatarNamePanel,EmailPanel,PasswordPanel,ChangePasswordForm,SetPasswordForm}.tsx`, `frontend/features/profile/constants.ts`, `frontend/app/profile/page.tsx`
App: frontend
Last updated: 2026-08-01 (Feature 55)

Page shell: `mx-auto flex max-w-2xl flex-col gap-6 px-6 py-8`, `h1 text-2xl font-semibold text-text-primary sm:text-3xl` — same header recipe as `FavoritesPageContent`/`BookingsPageContent`, narrower `max-w-2xl` since this page is a single column of stacked forms, not a card grid or list.
Panel shell (all three sections): the exact locked **Panel** pattern (`overflow-hidden rounded-2xl border border-border-default bg-surface`, header `border-b border-border-default px-5 py-4` with a plain `h2 text-lg font-semibold text-text-primary`, body `p-5`) — same shell `BookingSummaryCard` established, reused unmodified rather than inventing a variant.
Avatar Picker (new pattern — first of its kind): a `flex flex-wrap gap-3` row of `h-14 w-14 shrink-0 overflow-hidden rounded-full border-2` buttons, each showing a DiceBear-generated SVG (`img`, `object-cover`, empty `alt` since the button carries its own `aria-label="Use {seed} avatar"`). Selected state: `border-accent-primary` (vs `border-border-default hover:border-border-subtle` unselected) plus a small `absolute right-0 bottom-0 h-5 w-5 rounded-full bg-accent-primary text-white` badge holding a `CheckIcon` — same corner-badge mechanic as the Feature 54 selected/checked treatment, just circular here to sit on a circular avatar. `aria-pressed` reflects selection. **Any future avatar/photo-selection grid should reuse this exact button shape** — ring color for selection, corner check badge, `h-14 w-14` circle.
Verified/Not-verified badges (EmailPanel): reuses the exact **Booking Status Badge** shape (`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium`) with `border-success/20 bg-success-dim text-success` for Verified (+ `CheckCircle2Icon`) and `border-warning/20 bg-warning-dim text-warning` for Not verified — first reuse of that badge shell for something that isn't a booking status, confirming it's a general-purpose small-status-pill pattern, not booking-specific.
PasswordPanel branching: not a visual pattern but a load-bearing one — renders `ChangePasswordForm` or `SetPasswordForm` based on `authClient.listAccounts()` finding a `credential`-provider account, never on whether a `google` account is linked. Both forms share the same field/button classes as `ResetPasswordForm` (`h-10 rounded-xl border-border-default bg-subtle px-3 ...` inputs, `h-9 rounded-xl bg-accent-primary px-4 font-medium text-white hover:bg-accent-hover` submit).
Links row (bottom of page, not a Panel): plain `flex items-center gap-4 px-1 text-sm` holding two `Link`s (`text-text-secondary transition-colors hover:text-text-primary`) to `/bookings` and `/favorites` — deliberately not styled as buttons or cards; this is the "quiet links" requirement from `iteration-plan.md`.
Problems solved: better-auth's `setPassword` endpoint (used when no credential account exists yet) is `serverOnly` in the library — unreachable from `authClient` in the browser. Added a small dedicated backend route (`POST /users/set-password`, `requireAuth` → `auth.api.setPassword`) rather than trying to route it through better-auth's own client-facing surface. If any future feature needs another `serverOnly` better-auth endpoint from the frontend, this is the pattern: a thin Express route that calls `auth.api.<endpoint>` directly, not an attempt to expose it via `authClient`.
...
```
Box:   relative shrink-0 overflow-hidden bg-subtle aspect-[4/3]
Image: absolute inset-0 h-full w-full object-cover
```

Locked 2026-07-28 (bug found on `/search`, see `HotelCard` above). Any photo that must occupy a fixed, image-independent slot uses these two class strings together. The image must be **`absolute inset-0`**, not in flow: `h-full` on an in-flow child resolves against a parent whose height comes only from `aspect-ratio`, which is not a definite height, so the percentage falls back to `auto` — the image then takes its own intrinsic ratio and *grows the box past its aspect ratio*, since `aspect-ratio` yields to content. Landscape photos hide the bug; portrait ones expose it. `bg-subtle` on the box gives a neutral placeholder while the photo loads or when there is none. Render the `<img>` conditionally rather than passing `src={url ?? ""}` — an empty `src` makes the browser re-request the current page URL and paint a broken-image icon over the placeholder. In a `sm:flex-row` list card, pair the row-direction width with `sm:aspect-auto`: as a flex item the box is stretched to the card's height anyway, so leaving a competing `aspect-[4/3]` on it is misleading.

Adopted app-wide 2026-07-28. Every photo box in `frontend/` now follows it: `HotelCard`, `HotelCardSkeleton`, `FavoritesCard`, `SimilarHotelCard`, `CompareTable`, `BookingListCard`, `RoomTypeCard`, `TrendingDestinations` (`aspect-[3/4]`), `HotelGallery` (`aspect-[16/9]` — the box also had to gain `relative`, which it lacked). Measured in the running app at 1440px and 390px: every box now holds its ratio against portrait source photos that previously broke it — the 517×768 gallery photo renders 1104×621 in its 16/9 box rather than blowing it out, and Similar Hotels/Compare/Trending grids are uniform across mixed portrait and landscape seed images. Boxes with a fixed `h-*`/`w-*` instead of an aspect ratio are **not** affected and were deliberately left alone (`HotelGallery`'s `h-20 w-28` thumbnails, `BookingSummaryCard`'s `h-16 w-20`, `CompareTray`/`CompareSearchBox`/`ReviewListItem` avatars) — a definite parent height makes the child's `h-full` resolve correctly, so the in-flow form is fine there.
...
Both apps now have a shared `components/common/EmptyState.tsx` (icon/heading/body/action props, `frontend-admin`'s also takes an optional `iconClassName` — see below). `frontend` usages: search ("No hotels match these filters"), favorites, compare, bookings list, hotel-details room selection + not-found, checkout not-found, booking detail not-found, review page not-found/not-eligible, reviews section. `frontend-admin`'s `components/common/EmptyState.tsx` (added retroactively, consolidating three previously hand-rolled call sites — **Feature 29 follow-up**): `HotelsListPage` (`Building2` icon), `BookingsListPage` (`CalendarRange` icon for "No bookings found", plus its distinct error-state variant — `AlertTriangle` icon with `iconClassName="text-error"`, the reason the admin component needs the extra prop `frontend`'s doesn't), `RoomTypesSection` (`BedDouble` icon — deliberately matching `frontend`'s `BedDoubleIcon` choice for the same "no room types/rooms" concept, first explicit cross-app icon-consistency call). Any new `frontend-admin` empty/error state should import this component rather than hand-rolling the classes again.
...
**Radius always matches the real element it stands in for** — not a fixed `rounded-xl` everywhere. `rounded-2xl` for large image/hero/panel blocks, `rounded-lg` for small thumbnails (matching `HotelsListPage`/`BookingsListPage`'s real `size-10 rounded-lg` tiles), `rounded-full` for status-pill/avatar-circle placeholders, plain `rounded-xl` for text bars and buttons. Established by `HotelDetailsSkeleton`/`CheckoutSkeleton` (Features 12/21), confirmed and extended across all 7 of Feature 28's new skeletons (`HotelCardSkeleton`, `SearchResultsSkeleton`, `FavoritesSkeleton`, `CompareSkeleton`, `BookingsListSkeleton`, `HotelsTableSkeleton`, `BookingsTableSkeleton`, `DashboardSkeleton`) — 9 components now share this exact recipe with no deviation. No shared `<Skeleton>` primitive exists; each is a bespoke, hand-shaped component living beside the real component it stands in for.
