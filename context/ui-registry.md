...
App: frontend
Last updated: 2026-07-11 (Feature 17)
...
Right side: `flex items-center gap-3` — Favorites icon link, then `AccountMenu`/login CTA
Favorites icon: plain `<Link href="/favorites" aria-label="Favorites">`, `flex h-9 w-9 items-center justify-center rounded-xl text-text-muted transition-colors hover:bg-subtle hover:text-text-secondary`, `HeartIcon` (`h-5 w-5`, `strokeWidth={1.5}`) — no count badge, unlike the Compare icon `ui-rules.md` still calls for (that badge is Compare-specific, Feature 18's concern, not built yet)
CTA (logged out): Primary Button pattern via `render={<Link href="/login" />} nativeButton={false}` on the shadcn `Button` primitive
Notes: Server Component — calls `lib/get-server-session.ts` (forwards the incoming request's `cookie` header directly to the backend's `get-session`, since Server Components can't rely on the browser-only rewrite proxy). Renders `AccountMenu` when a session exists, the Log-in button otherwise. The Favorites icon is unconditional (guests can favorite too, via the session-cookie flow), not gated on login. Compact nav search bar and Compare icon are still not built.
...
App: frontend
Last updated: 2026-07-11 (Feature 17 — favorite toggle added to header block)
