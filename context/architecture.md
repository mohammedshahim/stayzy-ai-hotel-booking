...
**`AI_REQUEST_TIMEOUT_MS` is one global ceiling for every AI call, raised from 20s to 45s in Feature 41.** Not a per-feature override: `postToAgent`'s `timeoutMs` parameter remains available but nothing uses it. Cached features lose nothing by waiting longer (the ceiling is only reached on a miss), while smart search has no cache and pays full generation cost on every request — a measured extraction hit 17.7s, about two seconds under the old limit. 45s sits below the ~60s cut a production proxy typically imposes; **raising it further requires checking that proxy first**. `trust proxy` was the other open question here and is now settled — see the Invariants section.
...
- **Every tool lives in its own surface's `graphs/<feature>/tools/`, and no tool is shared** (added Feature 45 as a shared-if-both rule, **inverted in Feature 48** — `src/tools/` no longer exists). Each surface owns a full copy of every tool it binds, including the near-identical `SearchHotels`/`GetHotelDetails` the two both have. Two reasons, one safety and one behavioural: no booking, cancel, favorite, or review tool is even importable from where the widget builds its `TOOL_SCHEMAS`, so the widget's restriction is structural rather than a matter of discipline (verified live — asked "what are my bookings?", the widget calls no tool and says it cannot see them); and **a tool's docstring is that surface's prompt**, so retuning one surface's wording must not silently move the other's. Only non-tools stay shared: `graphs/outcome.py` (the `ToolOutcome` type) and `graphs/describe.py` (phrasing helpers).
...
**Added 2026-07-25 (Feature 48).** The chat-history sidebar needed three read routes on `backend/`, all additive to `routes/ai.routes.ts`: `GET /ai/chat/assistant/sessions`, `GET /ai/chat/assistant/sessions/:id`, and `POST /ai/chat/assistant/session/end`. None reaches the model, so none carries `chatRateLimit`. Two rules they establish:

- **`ended_at` means something different per feature.** For `widget` it means the conversation is over; for `chatbot` it means only "this is no longer the thread you land on" — every session a user owns stays writable forever, and the write path deliberately does not check it. The partial unique index still allows exactly one row with `ended_at IS NULL` per `(user, feature)`, which is what makes "the landing thread" well-defined and what "New chat" ends.
- **Every session route is ownership-checked and 404s otherwise**, including the per-session read. `findSessionOwner` is the single check; a session id is a uuid a caller could guess, and these routes return a full conversation.
...
- **The user session cookie is host-only unless `COOKIE_DOMAIN` says otherwise, and production needs it set.** better-auth writes no `Domain` attribute by default, so the cookie issued through `frontend/`'s `/api/auth` rewrite is scoped to the frontend host alone and is *not* sent to the API host — every authenticated `api-client.ts` call 401s. This never shows up in dev because cookie scope ignores the port, so `localhost:3000`'s cookie reaches `localhost:4000` unaided. `COOKIE_DOMAIN` (the shared parent, `.example.com`) turns on `advanced.crossSubDomainCookies` on the **user** instance only; the admin instance needs nothing, since its cookie is both set and sent on the API host, and its separate `admin` prefix keeps the two from colliding there.
...
- `agent/` never writes to a product table. Every mutation goes through `backend/src/routes/internal/*`, which are thin wrappers around existing services and contain no new business logic. **A guard the public route enforces from `req.user` does not survive that wrapping** — the internal route sees only an acting user id. `POST /bookings`'s `emailVerified` check is the live example: the internal route deliberately omits it, and the public `/ai` route that reaches the mutating tools owns it instead (Feature 46 → 47).
- **A mutating tool never executes without an `interrupt()` confirmation, and the `interrupt()` is inside the tool runner** rather than the node that dispatches it. Because resuming re-runs the entire node, a runner only reads before its pause, and a node dispatches at most one mutating tool call — otherwise a committed sibling call re-commits on resume. Established in Feature 46.
...
- **`trust proxy` is a hop count from `TRUST_PROXY`, defaulting to 0.** Settled in Phase 16, having been deliberately left open since Feature 38: IP keying only works if Express sees the real client address, and setting `trust proxy` without knowing the topology is strictly worse than leaving it — it makes `X-Forwarded-For` forgeable and the limiter bypassable by anyone who sends a header. The env var makes the count a deployment fact rather than a code constant. **The number is how many proxies actually sit in front of the app, not a boolean.** One CDN in front of the host is 2, not 1, and getting it wrong silently disables `aiRateLimit` in one direction or opens it to forgery in the other.
...
- **`agent/.env` reaches `os.environ` only through `load_dotenv()` in `main.py`.** `pydantic-settings` parses the file into `Settings` and never populates the process environment, so any library reading `os.environ` directly sees nothing without that call — LangSmith is the live case, and it fails *silently*, tracing simply never starting. Deleting the line as redundant is the obvious mistake. Anything reachable through `Settings` should still go through `Settings` (Feature 50).
- **Tracing is off unless `LANGSMITH_TRACING` is set, and a trace carries the whole transcript** — a user's real bookings, names and dates, on a third party's infrastructure. That switch is the privacy control; redaction (`LANGSMITH_HIDE_INPUTS`/`HIDE_OUTPUTS`) is available and deliberately unused, because hiding the prompt hides the only evidence for the tool-docstring bug class. Enabling it in production is a decision to make explicitly, not a default to inherit (Feature 50).
