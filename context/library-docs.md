...
- **`AGENT_DATABASE_URL` must point at a session-mode pooler, or no pooler at all — never a transaction pooler.** `checkpointer.py` opens its pool with `prepare_threshold: 0`, which prepares every statement; a transaction pooler hands the next statement to a different backend that has never seen the prepared name. On Supabase this means Supavisor on `:5432` and **not** `:6543`. The direct host is not an escape either — it is IPv6-only on free projects, and Render's outbound is IPv4
