...
**Current feature:** 10 Recent Searches + Search Suggestions
**Next up:** 10 Recent Searches + Search Suggestions — read `build-plan.md`'s section for it (`recent_searches` written on every search, scoped to guest cookie or logged-in user; search-while-typing suggestions from previously searched destinations plus known hotel/city names; guest→account merge on login, same pattern Feature 17 will use for favorites).
...
**Latest completed addition:** 09 Search Backend Wiring — 2026-07-07.
