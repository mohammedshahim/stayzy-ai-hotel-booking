...
**Current feature:** 09 Search Backend Wiring
**Next up:** 09 Search Backend Wiring — read `build-plan.md`'s section for it (`GET /search` with destination match, date-range availability filtering via a new `availability.service.ts`, sidebar filters, `sort` parameter, pagination; each result includes lowest available price + `location` for Map view). This is the first feature that needs real availability math (`room_types.total_inventory`/`rate_overrides.available_override` minus overlapping non-cancelled bookings) — but `bookings` doesn't exist as a feature yet (Phase 5), so availability for now reduces to inventory minus rate-override closures only, no booking overlap to subtract.
**Blocking issues:** None. Real S3 credentials (`S3_BUCKET`/`S3_REGION`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY`) confirmed working (developer-tested) — image upload is fully exercised, including room type images in Feature 08.
**Latest completed addition:** 08 Admin Room Type CRUD — 2026-07-07.
