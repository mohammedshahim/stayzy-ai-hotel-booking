...
- `tools/` — tool schemas and their runners, always at `graphs/<feature>/tools/`. **No tool is shared between surfaces; there is no top-level `src/tools/`.** A surface owns every tool it binds, even where two surfaces have a near-identical one, because a tool's docstring is a per-surface prompt and tuning one must never move the other. A tool returns a `ToolOutcome` — text for the model, id maps for the graph — and never touches `httpx` itself. *Reversed in Feature 48 (the rule previously put shared tools in `src/tools/`): a chatbot importing widget tool modules made both graphs hard to read, and the two surfaces had drifted to different runner signatures behind the shared name. What stays shared is only what is **not** a tool — `graphs/outcome.py` (the `ToolOutcome` type) and `graphs/describe.py` (phrasing helpers). Duplicating a type is a bug farm; duplicating a prompt is the point.*
...
**frontend/** — `next`, `react`, `better-auth` (client), `@stripe/stripe-js`, `@stripe/react-stripe-js`, `tailwindcss`, `shadcn/ui` components, `lucide-react`, `react-day-picker` + `date-fns` (shadcn's `Calendar` primitive and its date-math peer dependency, added in Feature 05 for the homepage date-range picker), `react-map-gl` + `mapbox-gl` (Map view on `/search`, added in Feature 06 — `mapbox-gl` ships its own TypeScript types, no `@types/` package needed), `react-markdown` (assistant replies are model-written markdown, added Feature 44) + `remark-gfm` (the model writes GFM tables when comparing hotels and commonmark renders them as raw pipes, added Feature 48), `@base-ui/react` (the shadcn primitives' own dependency; Feature 48 uses its `drawer` directly for the `/assistant` session list)
