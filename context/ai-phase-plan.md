...
- `POST /internal/bookings`, `POST /internal/bookings/:id/cancel`, `POST /internal/bookings/:id/review`, `POST /internal/favorites` — added in Feature 46, the four the mutating tools needed. Each resolves `req.actingUserId` and calls the same service the public route does, reusing `createBookingSchema`, `writeReviewSchema`, and `addFavoriteBodySchema` through `validateRequest`. **The one guard that does not carry over is the public create-booking route's `emailVerified` check**, which reads `req.user` — deliberately left out rather than turning a thin wrapper into a user lookup. Feature 47's chatbot route owns it instead; see Phase 14 below.
...
*Built 2026-07-25 (Feature 46), with four things worth carrying forward:* **`interrupt()` lives inside the tool runner**, not in the graph — `_preview` (reads only) → `confirm(...)` → commit — so the gate cannot be forgotten when Feature 47 assembles the agent. **Mutating tools resolve a name by re-fetching, never from an id map**, which sidesteps `room_type_ids` being keyed by bare room name (it collides across hotels) and gets a fresh price and inventory read at the moment of confirmation. **One generic confirmation envelope** `{action, title, lines: [{label, value}], confirm_label}` for all four, resumed with `{"approved": bool}`, so Feature 48 renders a single card. **The checkout link rides on `ToolOutcome.action` as a relative `/checkout/<id>` path**, never in model-facing text — `agent/` therefore needs no `FRONTEND_URL`. Tools went where their domain lives (`AddFavorite` → `account_tools.py`, `WriteReview` → `review_tools.py`), not all into `booking_tools.py`. See the Feature 46 entry in `progress-tracker.md`.

**Two obligations this hands Feature 47, both correctness bugs if missed:** the chatbot route must reject an unverified email (`POST /bookings` checks `emailVerified` in its controller; `requireAuth` does not, and `/internal` only receives a user id), and the tool node must dispatch **at most one mutating tool call per node execution**, because `interrupt()` resumes by re-running its node from the start and would re-commit an earlier sibling call.
...
*Built 2026-07-25 (Feature 47), reaching further than the line above: it ships the route too, so Feature 48 is pure React.* **Both of 46's obligations are discharged** — `403 email_not_verified` on message *and* decision turns before the model is reached, and a tool node that runs every read but at most one mutating call. **One route, `message` XOR `decision`**, the decision resuming with `Command(resume={"approved": bool})`; `sessionId` is optional and ownership-checked so a session list needs no contract change. **A message sent to a paused thread is refused with 409**, because LangGraph re-pauses rather than resuming and the alternative interleaves messages illegally — so 48 must disable the composer while a card is up. **`GET /ai/chat/assistant/pending`** makes a pause survive a reload. **An unseen hotel name resolves by searching but only on an exact name match**, so a loose name cannot become the wrong booking. New files: `graphs/chatbot/{state,prompts,nodes,graph}.py`, `api/chatbot_routes.py`, `api/chat_replies.py`, `tools/resolve.py`. See the Feature 47 entry in `progress-tracker.md` for the full SSE contract 48 consumes.
