...
import { useDeleteHotelImageMutation, useReorderHotelImagesMutation, useUploadHotelImageMutation } from "@/features/hotels/hotelsApi";
import type { HotelImage } from "@/features/hotels/types";
...
export function HotelImagesManager({ hotelId, images }: Props) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [draggedId, setDraggedId] = useState<string | null>(null);
  const [uploadImage, { isLoading: isUploading }] = useUploadHotelImageMutation();
  const [reorderImages] = useReorderHotelImagesMutation();
  ...
  async function handleFileSelected(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    try {
      await uploadImage({ hotelId, file }).unwrap();
    } catch {
  ...
  async function handleSetMain(imageId: string) {
    ...
    try {
      await reorderImages({ hotelId, imageIds, mainImageId: imageId }).unwrap();
    } catch {
  ...
  async function handleDelete(imageId: string) {
    try {
      await deleteImage({ hotelId, imageId }).unwrap();
    } catch {
  ...
  async function handleDrop(targetId: string) {
    ...
    try {
      await reorderImages({ hotelId, imageIds: ids, mainImageId }).unwrap();
    } catch {
